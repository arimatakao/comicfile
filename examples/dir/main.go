...
func main() {
	create := flag.Bool("c", false, "create the file directory")
	read := flag.Bool("r", false, "read the file directory")
	flag.Parse()
	...
	if *create {
		...
		if err := chapter.WriteOnDiskAndClose(".", filePath, metadata.Metadata{}, ""); err != nil {
			log.Fatal(err)
		}
