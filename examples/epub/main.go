...
const filePath = "file.epub"

func main() {
	createMode := flag.Bool("c", false, "create file.epub")
	readMode := flag.Bool("r", false, "read file.epub")
	flag.Parse()
...
func create() error {
	chapter, err := comicfile.NewContainer(comicfile.EPUB_EXT)
	if err != nil {
		return err
	}
	...
	if err := chapter.WriteOnDiskAndClose(".", "file", exampleMetadata(), ""); err != nil {
		return err
	}
